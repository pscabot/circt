...
static FailureOr<sim::DPIModuleType>
getDPISignature(Context &context,
  ...
  return sim::DPIModuleType::get(context.getContext(), ports);
}
...
Context::declareCallableImpl(const slang::ast::SubroutineSymbol &subroutine,
  ...
  if (options.emitMooreDPICall && !subroutine.thisVar &&
    ...
    if (failed(dpiSig))
      return nullptr;
    auto dpiModType = *dpiSig;
    auto dpiOp = moore::DPIFuncOp::create(
        builder, loc, StringAttr::get(getContext(), qualifiedName), dpiModType,
    ...
    lowering->functionType = dpiModType.getFunctionType();
